...
curl -fsSL "https://api.github.com/repos/MikeWang000000/FakeSIP/releases/latest" -o "$CURDIR/latest.json" || exit 1
...
for ARCH in "x86_64"; do
    if [ "$ARCH" = "x86_64" ]; then
        FILE_HASH=$(jq -r '.assets[] | select(.name=="fakesip-linux-x86_64.tar.gz") | .digest' "$CURDIR/latest.json" | sed 's/^sha256://')
        if [ -z "$FILE_HASH" ]; then
